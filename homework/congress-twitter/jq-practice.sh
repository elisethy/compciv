...
echo '6.'
cat data-hold/legislators-current.json | \
jq '.[] .bio .religion' | sort | uniq -c | sort -r
echo '7.'
cat data-hold/legislators-current.json | \
jq --raw-output '.[] .terms[0] .start' | sort | \
cut -d '-' -f 1 | grep -c '2015'
echo '8.'
cat data-hold/legislators-current.json | jq '.[] .terms[0] .start' | sort | tr -d '"' | head -n 1 | cut -d '-' -f 1   
echo '9.'
cat data-hold/legislators-current.json | \
jq --raw-output '.[] .terms[-1:][0].party' | grep -c 'Republican'
echo '10.'
cat data-hold/legislators-current.json | \
jq --raw-output '.[] .terms[-1:][0].party' | grep  -v 'Republican' | grep -c  -v 'Democrat'
echo '11.'
cat data-hold/legislators-current.json | \
...
echo '17.'
cat data-hold/congress-twitter-profiles.json | \
jq '.[] .followers_count' | sort -n -r | head -n 1
echo '18.'
cat data-hold/congress-twitter-profiles.json | \
